navpage<template>
	<div class="container-fluid px-3 hate" v-if="nav.show_about == true">
		<div class="fs-1 my-1">
			Author:
			<span class="text-warning">OrzOGC</span>
		</div>
		<div class="fs-1 my-1">活的精彩,修的痛快,Fix On RoaD</div>
		<div class="fs-1 my-1 text-center">違規魔人都去死一死啦幹</div>
		<div class="fs-1 my-1 text-end">臉皮都比我阿嬤的腳底皮還厚啦幹</div>
		<div class="fs-1 my-1 text-center">三寶都他媽的去吃屎啦幹</div>
		<div class="fs-1 my-1">龜仔開那麼慢還不滾去騎勁戰啦幹</div>
		<div class="fs-1 my-2 text-center">他媽的南部鄉下彰化貓奴就是操你娘的沒水準啦幹</div>
		...
		<div class="text-black lh-1 fuck fuck-4" :class="fuck_class[3]">啦</div>
	</div>
</template>
...
function toggle(flag) {
	if (fuck_class.value[flag] == "") {
		...
		fuck_class.value[flag] = "";
	}
}

function delay() {
	setTimeout(async () => {
		if (nav.value.show_about == true) {
